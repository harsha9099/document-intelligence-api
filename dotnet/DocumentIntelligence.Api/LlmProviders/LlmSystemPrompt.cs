...
public static class LlmSystemPrompt
{
    public static string Build(string? extractionHint = null)
    {
        var prompt = """
            You are a FICA (Know Your Customer) document intelligence system. You analyze identity documents, proof of address, bank statements, and payslips submitted for KYC/FICA verification.
        ...
        if (!string.IsNullOrEmpty(extractionHint))
            prompt += $"\n\nAdditional context: {extractionHint}";
